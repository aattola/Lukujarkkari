 ...
 * **************/
  ...
  let storeValParam = function (name) {
  ...
  let categoriesParam = function (name) {
    let separatorPattern = /\s*,\s*/g;
    return function (val, params, curr) {
      storeParam(val, params, curr)
  ...
  let exdateParam = function (name) {
    return function (val, params, curr) {
      let separatorPattern = /\s*,\s*/g;
      curr[name] = curr[name] || [];
      let dates = val ? val.split(separatorPattern) : [];
      dates.forEach((entry) => {
          let exdate = new Array();
  ...
  let recurrenceParam = function (name) {
      return dateParam(name);
  }

  let addFBType = function (fb, params) {
    let p = parseParams(params);
  ...
  let freebusyParam = function (name) {
    return function(val, params, curr){
      let fb = addFBType({}, params);
      curr[name] = curr[name] || []
      curr[name].push(fb);
  ...
  return {
    ...
    parseICS(str){
      let self = this
			let line_end_type = self.getLineBreakChar(str)
      let lines = str.split(line_end_type=='\n'?/\n/:/\r?\n/)
      let ctx = {}
      let stack = []
